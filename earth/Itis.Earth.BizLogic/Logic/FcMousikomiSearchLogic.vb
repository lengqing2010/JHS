...
Public Class FcMousikomiSearchLogic
...
#Region "FC申込データ取得"
    ...
    Public Function GetMousikomiDataInfo(ByVal sender As Object, _
        ...
        '検索実行クラス
        Dim dataAccess As New FcMousikomiDataAccess
        '取得データ格納用リスト
        Dim list As New List(Of FcMousikomiRecord)
        ...
        Try
            ...
            '検索結果を格納用リストにセット()
            list = DataMappingHelper.Instance.getMapArray(Of FcMousikomiRecord)(GetType(FcMousikomiRecord), table, startRow, endRow)
    ...
    Public Function GetMousikomiDataRec(ByVal sender As Object _
        ...
        If dTblResult.Rows.Count > 0 Then
            ' 検索結果を格納用リストにセット
            recResult = DataMappingHelper.Instance.getMapArray(Of FcMousikomiRecord)(GetType(FcMousikomiRecord), dTblResult)(0)
        End If
        Return recResult
    End Function
#End Region
...
#Region "FC申込データ更新"
    ...
    Public Function saveData(ByVal sender As Object, _
        ...
        '排他用データアクセスクラス
        Dim clsMousikomiDataAcc As New FcMousikomiDataAccess
        Dim recResult As New FcMousikomiRecord 'レコードクラス
        Dim recTmp As New FcMousikomiRecord '作業用
        'SQL文自動生成インターフェイス
        ...
        For Each recTmp In listData
            If recTmp.Status = EarthConst.MOUSIKOMI_STATUS_MI_JUTYUU Then '未受注時
                recType = GetType(FcMousikomiSyuuseiMijytyRecord)
            ElseIf recTmp.Status = EarthConst.MOUSIKOMI_STATUS_ZUMI_JUTYUU Then '受注済時
                recType = GetType(FcMousikomiSyuuseiJytyzumiRecord)
            ElseIf recTmp.Status = EarthConst.MOUSIKOMI_STATUS_HORYUU_JUTYUU Then '受注保留時
                recType = GetType(FcMousikomiSyuuseiJytyHoryuuRecord)
            Else
                Return False
                Exit Function
            End If
                ...
                For Each recTmp In listData
                    ...
                    If recTmp.MousikomiNo <> Long.MinValue Then 'UPDATE
                        ...
                        If recResult.UpdDatetime <> recTmp.UpdDatetime Then
                            ' 排他チェックエラー
                            mLogic.CallHaitaErrorMessage(sender, recResult.UpdLoginUserId, recResult.UpdDatetime, "FC申込テーブル")
                            Return False
                        End If
                        ...
                        '更新用文字列とパラメータの作成
                        strSqlString = IFsqlMaker.MakeUpdateInfo(recType, recTmp, listParam, GetType(FcMousikomiRecord))
...
#Region "新規受注処理"
    ...
    Public Function checkInput(ByVal jibanRec As JibanRecordBase) As Boolean
        ...
        End If
        '番号
        If cbLogic.GetDisplayString(jibanRec.HosyousyoNo) = String.Empty Then
            errMess += Messages.MSG013E.Replace("@PARAM1", "番号")
        End If
        '担当調査会社コード(事業所コード)
        If cbLogic.GetDisplayString(jibanRec.TysKaisyaCd) = String.Empty OrElse cbLogic.GetDisplayString(jibanRec.TysKaisyaJigyousyoCd) = String.Empty Then
            errMess += Messages.MSG013E.Replace("@PARAM1", "担当調査会社")
        End If
        '登録番号(加盟店コード)
    ...
    ''' <summary>
    ''' FC申込データを元に地盤データ作成を行います。[EARTH上に受注処理を行なう]
    ''' ※FC申込テーブル.区分と番号には採番された値をセットし、ステータスを更新する
    ''' </summary>
    ''' <param name="listMi">FC申込レコード</param>
    ''' <returns>True or False</returns>
    ''' <remarks></remarks>
    Public Function saveDataJutyuu( _
                            ByVal sender As Object _
                            , ByRef listMi As List(Of FcMousikomiRecord) _
        ...
        Dim recTysKaisya As New TysKaisyaRecord
            ...
            Using scope As TransactionScope = New TransactionScope(TransactionScopeOption.Required, TimeSpan.FromMinutes(EarthConst.TRAN_TIMEOUT))
                ...
                For intCnt = 0 To listMi.Count - 1
                    ...
                    If recResult Is Nothing OrElse recResult.MousikomiNo = Long.MinValue Then
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, Messages.MSG147E.Replace("@PARAM1", "FC申込データ取得")), 1)
                        Return False
                    End If
                    ' ステータスチェック
                    ...
                    End If
                    '地盤データ(区分、番号)チェック
                    If jLogic.ExistsJibanData(recResult.Kbn, recResult.HosyousyoNo) Then
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, "該当の物件情報が既に地盤テーブルに存在します。\r\n[区分]" & recResult.Kbn & "\r\n[番号]" & recResult.HosyousyoNo & "\r\n"), 1)
                        Return False
                    End If
                    '商品コード1(倉庫コード='100')の存在チェック
                    strSoukoCd = cbLogic.pf_getBunruiCd(recResult.SyouhinCd)
                    If strSoukoCd <> EarthConst.SOUKO_CD_SYOUHIN_1 Then
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, "商品コード[" & recResult.SyouhinCd & "]が存在しないか、分類が異なります。\r\n"), 1)
                        Return False
                    End If
                    '調査方法NOのチェック
                    ...
                    End If
                    '調査会社コードのチェック
                    recTysKaisya = tysKaisyaLogic.getTysKaisyaInfo(recResult.TantouTysKaisyaCd, recResult.TantouTysKaisyaJigyousyoCd)
                    If Not recTysKaisya Is Nothing AndAlso recTysKaisya.Torikesi = 0 Then
                    Else
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, "担当調査会社(事業所)コード[" & recResult.TantouTysKaisyaCd & "(" & recResult.TantouTysKaisyaJigyousyoCd & ")]が存在しないか、取消されています。\r\n"), 1)
                        Return False
                    End If
                    ...
                    With recTmp
                        .Kbn = recResult.Kbn
                        .HosyousyoNo = recResult.HosyousyoNo
                        ...
                        If jLogic.InsertJibanData(sender, .Kbn, .HosyousyoNo, .UpdLoginUserId, 1, EarthEnum.EnumSinkiTourokuMotoKbnType.ReportJHS_FC) = False Then
                    ...
                    If intRet <> 1 Then
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, Messages.MSG147E.Replace("@PARAM1", "FC申込データ更新(番号、ステータス)")), 1)
                        Return False
                    End If
                    ...
                    If intRet <> 1 Then
                        mLogic.AlertMessage(sender, Me.SetErrMsg(strTmpMousikomiNo, Messages.MSG147E.Replace("@PARAM1", "地盤データ更新(FC申込データ移送)")), 1)
                        Return False
                    End If
    ...
    Public Function UpdateJibanData(ByVal sender As Object, _
        ...
        Try
            ...
            For intCnt = 1 To intMax
                ...
                '●物件名寄コードの自動設定
                If jibanRec.BukkenNayoseCd = String.Empty Then
                ...
                '****************************************************************************
                '商品の自動設定後に行なう
                cbLogic.ps_AutoSetHosyousyoHakJyjy(EarthEnum.emGamenInfo.FcMousikomiSearch, jibanRec)
                ...
                '*************************
                '更新用文字列とパラメータの作成
                updateString = upadteMake.MakeUpdateInfo(GetType(JibanRecordFcMousikomiJutyuu), jibanRec, list, GetType(JibanRecordBase))
