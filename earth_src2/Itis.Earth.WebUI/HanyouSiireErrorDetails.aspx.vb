...
Partial Public Class HanyouSiireErrorDetails
    ...
    Protected Sub SetInitData()
        Dim hanyouSiireBC As New HanyouSiireLogic
        ...
        dtKojKakakuInfo = hanyouSiireBC.GetHanyouSiireErr(strEdiDate, strSyoriDate, 100)
        '検索結果を設定する
        ...
        '検索結果件数を取得する
        Dim intCount As Integer = hanyouSiireBC.GetHanyouSiireErrCount(strEdiDate, strSyoriDate)
        '検索結果件数を設定する
    ...
    Private Sub btnCsvOutput_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCsvOutput.Click
        ...
        '工事価格エラーCSVデータを取得する
        dtHanyouSiireCsvInfo = hanyouSiireBC.GetHanyouSiireErr(ViewState("EdiJouhouDate"), ViewState("SyoriDate"), System.Configuration.ConfigurationManager.AppSettings("CsvDownMax"))
        'CSVファイル名設定
        Dim strFileName As String = System.Configuration.ConfigurationManager.AppSettings("HanyouSiireErrCsv").ToString

        Response.Buffer = True
        Dim writer As New CsvWriter(Me.Response.OutputStream, Encoding.GetEncoding(932), ",", vbCrLf)

        'CSVファイルヘッダ設定
        writer.WriteLine(EarthConst.conHanyouSiireErrCsvHeader)
        ...
        For Each row As Data.DataRow In dtHanyouSiireCsvInfo.Rows
            writer.WriteLine(row.Item("edi_jouhou_sakusei_date"), _
                                ...
                                row.Item("torikesi"), _
                                row.Item("kameiten_cd"), _
                                row.Item("tys_kaisya_cd"), _
                                row.Item("tys_kaisya_jigyousyo_cd"), _
                                row.Item("syouhin_cd"), _
                                row.Item("suu"), _
                                row.Item("tanka"), _
                                row.Item("zei_kbn"), _
                                row.Item("syouhizei_gaku"), _
                                SetTimeType(row.Item("siire_date"), True), _
                                SetTimeType(row.Item("denpyou_siire_date"), True), _
                                row.Item("siire_keijyou_flg"), _
                                SetTimeType(row.Item("siire_keijyou_date"), True), _
                                row.Item("kbn"), _
                                row.Item("bangou"), _
