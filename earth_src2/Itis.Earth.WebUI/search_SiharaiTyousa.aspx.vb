...
Partial Public Class search_SiharaiTyousa
    ...
    ''' <history>
    ''' <para>2010/05/20　馬艶軍(大連情報)　新規作成</para>
    ''' </history>
    ...
    Protected Sub MakeJavaScript()
        ...
        With csScript
            ...
            If ViewState("strKbn") = "支払集計先事業所" Then
                .AppendLine("       alert('支払集計先事業所コードと調査会社コードが異なるため、設定できません。');")
            Else
                .AppendLine("       alert('支払明細集計先事業所コードと調査会社コードが異なるため、設定できません。');")
            End If
